...
- app.py owns sentence-boundary chunking and will call speak_sentence()
  on each completed sentence while Nimbus is still generating subsequent tokens.
- No cross-sentence queueing -- each call is independent.
...
- Full cancellation (WebSocket close) is flag-based only.
...
class TTS(ABC):
    ...
    CartesiaSonicTTS (cloud streaming, ~150-250ms TTFB, expressive
    buddy voice). candidates: Pyttsx3TTS (local offline fallback),
    ...
    def speak_sentence(self, sentence: str) -> None:
        ...
        As Nimbus generates response tokens, app.py buffers until a sentence
        boundary (./!/?), then calls speak_sentence() on that chunk while
        Nimbus continues generating. Each call is independent -- there is no
        cross-sentence queueing, so overlapping calls will produce
    ...
    def stop(self) -> None:
        ...
        Sets a stop flag that newly-spawned speak() threads check at
        startup and on each chunk -- in-progress chunks already in the
        sounddevice buffer still play out.
        """
        ...
    ...
    def arm_first_chunk_callback(self, cb: Callable[[], None]) -> None:
        ...
        ``MODEL: streaming started`` and the moment the user actually
...
class CartesiaSonicTTS(TTS):
    """TTS using Cartesia Sonic-3 streaming.
    ...
    No fallback: if Cartesia is unreachable, the app is voiceless.
    A Pyttsx3TTS subclass is ~1 hour of work if that ever happens.
    """
    ...
    def __init__(
        ...
        # HTTP double-buffer. _sentence_queue feeds the prefetch
    ...
    def speak_sentence(self, sentence: str) -> None:
        ...
        stream Nimbus's response sentence-by-sentence while later sentences
    ...
    def stop(self) -> None:
        ...
        Six-pronged kill (the double-buffer extends the drain with epoch guard):
    ...
    def _do_speak(self, text: str, cancel: threading.Event) -> None:
        ...
        try:
            ...
            raise RuntimeError(
                ...
                "     fallback -- ~1 hour of work, see notes.\n"
                f"Underlying error: {type(exc).__name__}: {exc}"
            ) from exc
...
# --- ElevenLabsTTS (opt-in alternative to Cartesia) ---------------


class ElevenLabsTTS(TTS):
    """ElevenLabs Flash v2.5 streaming TTS as an opt-in alternative to
    Cartesia. Mirrors CartesiaSonicTTS's prefetch+playback
    ...
    def __init__(
        ...
        # prefetch+playback two-thread architecture, mirrors
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("Nimbus -- tts.py manual verification (Cartesia Sonic-3)")
    print("=" * 70)

    tts = CartesiaSonicTTS(api_key=CARTESIA_API_KEY)

    test_text = (
        "Hello, I am Nimbus. I am your voice AI buddy built on "
