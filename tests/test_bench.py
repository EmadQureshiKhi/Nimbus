"""Unit tests for tools.bench stats wrappers + MetricRow.

The scraping code (_scrape_latest_n, record_cmd, compare_cmd) is exercised
manually — we don't unit-test the CLI because it depends on real
~/.nimbus/debug/ folders.
"""
import numpy as np
...
def test_mann_whitney_less_detects_lower_after():
    ...
    from tools.bench import mann_whitney_less
...
def test_bootstrap_median_ci_contains_sample_median():
    ...
    from tools.bench import bootstrap_median_ci
    rng = np.random.default_rng(42)
...
def test_metric_row_summary_has_expected_keys():
    ...
    from tools.bench import MetricRow
    row = MetricRow(
