...
class TestCartesiaSonicTTSSpeak:
    ...
    def test_speak_sentence_queues_and_plays_sequentially(self):
        """multiple speak_sentence calls play sequentially via
    ...
    def test_prefetch_fires_before_previous_playback_completes(self):
        """the double-buffer design: HTTP request for sentence N+1 must start while N is still
...
class TestElevenLabsTTSSentenceQueue:
    ...
    two-thread architecture (the double-buffer design), NOT cancelling each other."""
