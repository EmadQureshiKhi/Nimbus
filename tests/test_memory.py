...
class TestRecord:
    ...
    def test_record_appends_second_interaction_and_updates_index(self, tmp_path, monkeypatch):
        ...
        assert md.count("## 2026-07-14") == 2

        apps = store.list_known_apps()
        assert apps[0]["interaction_count"] == 2
        assert apps[0]["first_seen"] == "2026-07-14 14:30"
        assert apps[0]["last_seen"] == "2026-07-14 15:45"
    ...
    def test_record_uses_same_timestamp_for_markdown_and_sqlite(self, tmp_path, monkeypatch):
        ...
        md = (tmp_path / "memory" / "excel.exe.md").read_text(encoding="utf-8")
        assert "First seen: 2026-07-14 15:30" in md
        assert "## 2026-07-14 15:30" in md

        apps = store.list_known_apps()
        assert apps[0]["first_seen"] == "2026-07-14 15:30"
        assert apps[0]["last_seen"] == "2026-07-14 15:30"
...
class TestListKnownApps:
    ...
    def test_list_known_apps_sorted_by_last_seen_desc_with_correct_dict_shape(
        ...
        times = [
            datetime(2026, 7, 14, 10, 0),  # a.exe first
            datetime(2026, 7, 14, 11, 0),  # b.exe second
            datetime(2026, 7, 14, 12, 0),  # c.exe third
        ]
        idx = [0]
