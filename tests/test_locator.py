...
Covers the two-stage grid locator against Nimbus's sync code style + coord
conventions + AIClient context-manager interface (returns _StreamingResponse,
not async iterator).
...
class TestAskGridPick:
    ...
    def test_logs_distinct_message_on_llm_transport_failure(self, mocker):
        """When LLM call raises (Ollama timeout, model not
    ...
    def test_logs_distinct_message_on_unparseable_reply(self, mocker):
        """When LLM responds but reply is unparseable
