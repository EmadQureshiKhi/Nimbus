...
class TestSanitizeAppName:
    ...
    def test_sanitize_lowercases_keeping_dot_exe(self):
        ...
        assert _sanitize_app_name("MyApp.EXE") == "myapp.exe"
...
class TestRecall:
    ...
    def test_recall_returns_empty_for_missing_file(self, tmp_path: Path):
        """No .md file for this app → ('', '') — Nimbus already knows."""
    ...
    def test_recall_returns_empty_when_kb_dir_does_not_exist(
        ...
        content, name = recall("myapp.exe", kb_dir=missing)
        assert content == ""
        assert name == ""
    ...
    def test_recall_returns_full_content_when_file_under_budget(
        ...
        from kb import recall
        body = "# MyApp KB\n\n## Plotting\nUse Chart > Add..."
        (tmp_path / "myapp.exe.md").write_text(body, encoding="utf-8")
        content, name = recall("MYAPP.EXE", kb_dir=tmp_path)
        assert content == body
    ...
    def test_recall_returns_empty_for_non_positive_max_chars(
        ...
        from kb import recall
        (tmp_path / "myapp.exe.md").write_text("hello", encoding="utf-8")
        assert recall("myapp.exe", kb_dir=tmp_path, max_chars=0) == ("", "")
        assert recall("myapp.exe", kb_dir=tmp_path, max_chars=-5) == ("", "")
        # Sanity: positive max_chars works on the same file.
        assert recall("myapp.exe", kb_dir=tmp_path)[0] != ""
    ...
    def test_recall_uses_default_kb_dir_when_override_is_none(
        ...
        path without touching the real ~/Documents/Nimbus Wiki/.
        """
        from kb import recall
