"""PTT latency benchmark harness — measure before/after PTT latency with stats.

Usage
-----
1. Record a baseline set of interactions:

       py -3.13 -m tools.bench record --label before --n 20
   ...
   and press Enter. It scrapes ~/.nimbus/debug/ for the N most-
   recent folders.)

2. Apply a change, then record again:

       py -3.13 -m tools.bench record --label after --n 20

3. Compare the two runs:

       py -3.13 -m tools.bench compare \\
           ~/.nimbus/bench/before.json \\
           ~/.nimbus/bench/after.json
...
BENCH_DIR = Path.home() / ".nimbus" / "bench"
DEBUG_DIR = Path.home() / ".nimbus" / "debug"
...
def _scrape_latest_n(n: int) -> dict:
    ...
    for folder in folders:
        ...
        model_start = _extract_timing(log_path, "MODEL: streaming started")
        ...
        if model_start is not None:
            metrics["model_first_token_ms"].append(model_start)
...
def record_cmd(label: str, n: int) -> None:
    """Prompt the user to run N real PTT interactions, then scrape ~/.nimbus/debug/."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="tools.bench",
        description="PTT latency benchmark harness for Nimbus.",
    )
